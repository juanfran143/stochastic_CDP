...
class Algorithms:
    ...
    def ConstructiveHeuristic_without_last_move_simulation(self, sol, simulation = simheuristic(20, 0.1), alpha = 0.9):
            sol.of = 0
            ...
            while (len(sortedDistList) != 0):
                ...
                #TODO
                lower, upper = simulation.fast_simulation(sol, nodeMinCap)
                relibility_good = True if alpha < lower else False
                #if alpha < upper and alpha > lower:
                #    simulation.simulations = 100

                if relibility_good: #I can delete the node because I have not violate the demand
                    nodeMinCap.used = True
                    del sol.selected[sol.selected.index(nodeMinCap.id)] #delete the node with minimum capacity of the solution
                    sol.nonUsedNodes.append(nodeMinCap.id)
                    sol.capacity -= nodeMinCap.capacity #substract the capacity of the node of the solution.
    ...
    def localSearch_simulation(self,sol, simulation = simheuristic(20, 0.1), alpha = 0.9):
        ...
        while improvement:
            ...
            for nOutSol in sol.nonUsedNodes:
                ...
                for nInSol in sol.selected:
                    distanceBetweenNodes = self.instance.distance[nInSol][nOutSol]
                    p = simulation.fast_simulation_localsearch(sol, nInSol, nOutSol)
                    if CurrentMinDistance >= distanceBetweenNodes or alpha < p:
                        ValidNode = -1 #not valid node to be included in the solution
                        break
                if ValidNode != -1:
                    improvement = True
